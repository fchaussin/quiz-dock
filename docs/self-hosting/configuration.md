...
| `HOST_LAN_IPS` | — | app | Comma-separated LAN IPs of the machine, for local instances where the container cannot see the host's interfaces (Docker Desktop). Offered as invitation addresses. The host can always type any address in the console. |
